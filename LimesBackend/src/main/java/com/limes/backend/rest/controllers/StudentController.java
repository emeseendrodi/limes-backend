...
public class StudentController extends AbstractController {
    ...
    public ResponseEntity getProfileData(@RequestHeader(HttpHeaders.AUTHORIZATION) @NotBlank String auth) {
        String email;
        try {
        ...
        weeks.stream().forEach(w -> {
            weekProgress.add(new ProgressionObjectModel(w.getName(), !progress.stream().anyMatch(f -> (f.getWeek_id() == w.getId() && !f.isExists()))));
        });
